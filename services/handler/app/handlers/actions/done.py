...
async def done_command_handler(bot, message, sql_session=None) -> None:
    ...
    players_count = bot.app.store.players.get_players_count(sql_session=sql_session,
                                                            session_id=session.id)
    ...
    if answered == players_count:
        ...
        await bot.send_message(message=f'Okay, all players doesnt`t give the answer, correct is: {answer}',
                               chat_id=message.chat.id)
